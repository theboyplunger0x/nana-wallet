...
## Signing-scope decision (2026-09-09, user-authorized)

User explicitly authorized enabling signer enrollment/payments with the provable per-transfer policy only (chain 5042002 + USDC contract + transfer <= 10 USDC + recipient allowlist + gas ceiling), leaving the rolling 50 USDC/3600s aggregate as a PENDING FEATURE. Implementation: removed the aggregation fail-closed gate from prepare/complete (embedded.ts); removed the active->unavailable demotion in getPermission (state now reported honestly); policy ships WITHOUT the aggregation condition; UI shows the pending hourly limit via HOURLY_LIMIT_PENDING note (aggregationReady:false) next to the overshoot caveat. Security retained: complete-readback proves owner (user_id filter) + exact stored policy id before activation; legacy grants whose attached signer policy differs stay pending (test); never auto-regrants revoked signers; preview/confirm/idempotency unchanged. 5 enrollment tests + cross-user test updated to the authorized semantics. Suites: backend 591/0, front 69/0, lint/tsc/build/evals 0.
