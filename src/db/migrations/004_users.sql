-- Local mirror of supabase/migrations/20260901000300_users.sql
...
GRANT SELECT, INSERT, UPDATE, DELETE ON users TO recipient_app;
...
CREATE OR REPLACE FUNCTION users_ensure_for_privy_did(p_privy_did TEXT, p_display_name TEXT DEFAULT NULL)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION users_ensure_for_privy_did(TEXT, TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION users_ensure_for_privy_did(TEXT, TEXT) FROM recipient_app;
...
ALTER TABLE recipients ADD CONSTRAINT recipients_user_id_users_fk
  FOREIGN KEY (user_id) REFERENCES users(id) NOT VALID;
ALTER TABLE conversations ADD CONSTRAINT conversations_user_id_users_fk
  FOREIGN KEY (user_id) REFERENCES users(id) NOT VALID;
