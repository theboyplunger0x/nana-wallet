...
const NETWORK = () => process.env.WDK_NETWORK
  ?? (process.env.IDENTITY_PROVIDER === 'privy' ? 'arc-testnet' : 'sepolia');
const WALLET = () => process.env.WDK_WALLET_NAME
...
const MODE = () => process.env.IDENTITY_PROVIDER === 'privy'
  ? 'live'
  : process.env.WDK_TOOLS_SOURCE === 'live' || process.env.WDK_TOOLS_SOURCE === 'circle-arc'
    ? 'live'
    : 'fixture';
