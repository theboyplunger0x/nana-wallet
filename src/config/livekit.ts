...
/**
 * Single source of truth for the agent dispatch name. The room token issuer
 * dispatches explicitly by this name (RoomConfiguration.agents) and the worker
 * must register under the SAME name (ServerOptions.agentName) or explicit
 * dispatch targets no registered worker.
 */
export function resolveDefaultAgentName(
  environment: NodeJS.ProcessEnv = process.env,
): string {
  return environment.LIVEKIT_AGENT_NAME?.trim() || "nani-agent";
}
...
export function readLiveKitTokenIssuerConfig(
  ...
  return {
    ...
    browserUrl:
      environment.LIVEKIT_BROWSER_URL?.trim() ||
      environment.LIVEKIT_URL!.trim(),
    apiKey: environment.LIVEKIT_API_KEY!.trim(),
    apiSecret: environment.LIVEKIT_API_SECRET!.trim(),
    defaultAgentName: resolveDefaultAgentName(environment),
    roomTokenTtlSeconds,
  };
}

export function readLiveKitPrivacyConfig(
  environment: NodeJS.ProcessEnv = process.env,
): LiveKitPrivacyConfig {
  for (const name of [
    "LIVEKIT_RECORDING_ENABLED",
    "AGENT_OBSERVABILITY_RECORDING",
  ]) {
  ...
  if (
    environment.LIVEKIT_RECORDING_ENABLED === "1" ||
    environment.LIVEKIT_RECORDING_ENABLED === "true"
  ) {
    throw new Error("LiveKit recording must remain disabled.");
  }
  if (
    environment.AGENT_OBSERVABILITY_RECORDING === "1" ||
    environment.AGENT_OBSERVABILITY_RECORDING === "true"
  ) {
