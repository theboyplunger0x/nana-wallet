...
- `users` table: internal UUID primary key, `privy_did` `UNIQUE`, display name, and timestamps. First-login provisioning via a `SECURITY DEFINER` function `users_ensure_for_privy_did(did, display_name)` called on the owner connection — no new `BYPASSRLS` role. Demo startup or the demo seed provisions `privy_did='demo'` with the configured `DEMO_USER_ID` before any user-scoped writes.
